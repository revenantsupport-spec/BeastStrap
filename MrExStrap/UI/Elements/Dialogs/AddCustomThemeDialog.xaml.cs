...
namespace BeastStrap.UI.Elements.Dialogs
{
    ...
    public partial class AddCustomThemeDialog : WpfUiWindow
    {
        ...
        private static void CreateCustomTheme(string name, CustomThemeTemplate template)
        {
            ...
            // Second line of defence behind PathValidator. Path.Combine does not normalise "..",
            // but Win32 does, so an escaping name would make the Delete below recurse over the
            // install root. Never delete anything that isn't demonstrably inside CustomThemes.
            if (!PathValidator.IsContainedIn(dir, Paths.CustomThemes))
                throw new ArgumentException($"Theme name '{name}' resolves outside the custom themes folder.", nameof(name));
