...
namespace BeastStrap.Models.Entities
{
    public class ActivityData
    {
        ...
        public long UniverseId
        {
            ...
            {
                _universeId = value;
                // Was calling LoadFromCache and throwing the result away, so UniverseDetails
                // stayed null until something else populated it.
        ...
        public async Task<string?> QueryServerLocation()
        {
            ...
            // try/finally so the permit can't leak. Every Release() used to sit inside a branch,
            // so a throw from the logging calls in the catch would strand the semaphore and
            // deadlock every later lookup on this instance.
            try
            {
                if (GlobalCache.ServerLocation.TryGetValue(MachineAddress, out string? cached))
                    return cached;
                ...
                catch (Exception ex)
                {
                    ...
                    // Deliberately NOT cached. This used to write the still-null `location` into
                    // the cache, and the TryGetValue above then returned "true, null" forever —
                    // one failed lookup permanently poisoned the location readout for that server
                    // with no retry and no TTL for the life of the process.
                    location = null;
                }
