...
using BeastStrap.Models.RobloxApi;
using DiscordRPC;
...
namespace BeastStrap.Integrations
{
    public class DiscordRichPresence : IDisposable
    {
        ...
        // Guards _messageQueue, which is enqueued from the ActivityWatcher log-reader thread and
        // drained from thread-pool threads.
        private readonly object _queueLock = new();

        // Serialises SetCurrentGame. OnGameJoin and OnGameLeave each kick off their own Task.Run,
        // and an in-universe teleport fires leave-then-join back to back. Run concurrently, the
        // "leave" pass could finish last and null out the presence the "join" pass had just built,
        // clearing rich presence while the user was still in a game.
        private readonly SemaphoreSlim _presenceLock = new(1, 1);

        // Set in Dispose. The log tail can outlive us by a moment, and touching the RPC client
        // after it is disposed throws ObjectDisposedException on a thread-pool thread.
        private volatile bool _disposed;

        // Kept so Dispose can detach them — otherwise these closures stay registered on the
        // ActivityWatcher (which lives longer) and keep calling back into a disposed client.
        private readonly EventHandler _onGameJoin;
        private readonly EventHandler _onGameLeave;
        private readonly EventHandler<Message> _onRpcMessage;
        ...
        public DiscordRichPresence(ActivityWatcher activityWatcher)
        {
            ...
            _onGameJoin = (_, _) => Task.Run(() => SetCurrentGame());
            _onGameLeave = (_, _) => Task.Run(() => SetCurrentGame());
            _onRpcMessage = (_, message) => ProcessRPCMessage(message);

            _activityWatcher.OnGameJoin += _onGameJoin;
            _activityWatcher.OnGameLeave += _onGameLeave;
            _activityWatcher.OnRPCMessage += _onRpcMessage;
        ...
        public async Task<bool> SetCurrentGame()
        {
            ...
            try
            {
                return await SetCurrentGameCore();
            }
            catch (ObjectDisposedException)
            {
                return false; // raced Dispose, nothing left to update
            }
            finally
            {
        ...
        private async Task<bool> SetCurrentGameCore()
        {
            ...
            // Drain the whole queue, not just the head. A game that calls SetRichPresence a few
            // times while we were still building the presence had every message after the first
            // sit here until the next Clear() threw them away.
            while (true)
            {
                Message? queued;
                ...
                lock (_queueLock)
                {
                    ...
                    queued = _messageQueue.Dequeue();
                }

                App.Logger.WriteLine(LOG_IDENT, "Processing queued message");
                ProcessRPCMessage(queued, false);
            }
        ...
        public void Dispose()
        {
            ...
            // Detach first. The ActivityWatcher outlives us and its log tail can still be holding
            // buffered lines — without this, a late "Replicator created:" fired SetCurrentGame at
            // a disposed RPC client and threw on a thread-pool thread.
            _disposed = true;
            ...
            {
                App.Logger.WriteException("DiscordRichPresence::Dispose", ex);
            }

            // _presenceLock is deliberately not disposed: a SetCurrentGame call can still be in
            // flight holding it, and its Release() in the finally would then throw. SemaphoreSlim
            // holds no unmanaged resource unless AvailableWaitHandle is touched, which it isn't.
