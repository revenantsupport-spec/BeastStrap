namespace BeastStrap.Integrations
{
    public class ActivityWatcher : IDisposable
    {
        ...
        /// </summary>
        /// <remarks>
        /// Written from the log-reader thread and read from UI threads. Take <see cref="HistoryLock"/>
        /// for anything more than a single indexed read, or use <see cref="SnapshotHistory"/> —
        /// enumerating this list directly while a game ends throws "collection was modified".
        /// </remarks>
        public List<ActivityData> History = new();

        public readonly object HistoryLock = new();

        /// <summary>A point-in-time copy of <see cref="History"/>, safe to enumerate across awaits.</summary>
        public List<ActivityData> SnapshotHistory()
        {
            lock (HistoryLock)
                return new List<ActivityData>(History);
        }
        ...
        /// <summary>
        /// Tails Roblox's client log and raises the join/leave/message events off it.
        /// </summary>
        /// <remarks>
        /// Every throw in here has to be caught in here. This is `async void`, launched
        /// fire-and-forget from Watcher.Run, so anything that escapes goes straight to
        /// AppDomain.UnhandledException and kills the watcher process — which owns the tray icon,
        /// Discord presence, the auto-close cleanup and the crash dialog. The user sees the app
        /// vanish with no explanation, so failing quietly and leaving the client alone is always
        /// the better outcome.
        /// </remarks>
        ...
        private async Task RunAsync()
        {
            ...
            if (String.IsNullOrEmpty(LogLocation))
            {
                ...
                while (true)
                {
                    ...
                    // FirstOrDefault, not First. The directory can exist with no Player log in it —
                    // a first-ever launch where Roblox made the folder before the file, or right
                    // after BanAsync's "clean traces" wipes %LocalAppData%\Roblox. First() threw
                    // InvalidOperationException there and took the whole watcher process down.
                    ...
                    if (newest is null)
                    {
                        App.Logger.WriteLine(LOG_IDENT, "No Roblox player log in the log folder yet, waiting...");
            ...
            while (!IsDisposed)
            {
                ...
                // Per-line guard. ReadLogEntry fans out to subscribers (Discord presence, the
                // watcher's app-close handler, the tray toast), and one of those throwing used to
                // end the tail loop for the rest of the session — activity tracking, presence and
                // crash detection all silently stopped after a single bad line.
                try
                {
                    ReadLogEntry(log);
                }
                ...
                {
                    App.Logger.WriteException(LOG_IDENT + "::ReadLogEntry", ex);
                }
            }
        }
        ...
        private void RecordRecentPlace()
        {
            ...
            try
            {
                ...
                // SaveMerged, not Save. This runs in the watcher, which lives for a whole play
                // session, and State also carries ActiveInstallProfileId/ActiveInstallVersionGuid.
                // A plain save here would write our session-old copy of those over whatever a
                // launch happening in parallel had just recorded, which is exactly the profile
                // bookkeeping the v420.43 install-swap fix depends on.
                App.State.SaveMerged(s => s.PushRecentPlace(Data.PlaceId, Data.UniverseDetails?.Data.Name));
            }
            catch (Exception ex)
            {
