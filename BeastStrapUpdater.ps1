...
.EXAMPLE
    # Ship a specific version instead of auto-bumping, and skip the rebuild:
...
# 1. version
# ----------------------------------------------------------------------------
$current = ((Select-String -Path $Csproj -Pattern '<Version>([^<]*)</Version>' | Select-Object -First 1).Matches[0].Groups[1].Value).Trim()
if ([string]::IsNullOrWhiteSpace($Version)) {
    $v = [Version]$current
    $Version = if ($v.Build -gt 0) { "$($v.Major).$($v.Minor).$($v.Build + 1)" } else { "$($v.Major).$($v.Minor + 1).0" }
}
$Tag = "v$Version"
...
Write-Host ("Release exe: {0} ({1} MB, version {2})" -f $Publish, [math]::Round($exeInfo.Length / 1MB, 1), $Version) -ForegroundColor Green
...
Write-Host "Uploading BeastStrap.exe ($([math]::Round($exeInfo.Length / 1MB, 1)) MB)..." -ForegroundColor Yellow
